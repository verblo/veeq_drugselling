...
local function isPedBlocked(ped)
    ...
    for _, blockedPed in ipairs(Config.BlockedPeds) do
        if pedModel == GetHashKey(blockedPed) then
            return true
        end
    end
    return false
...
local function handlePedInteraction(data, event)
